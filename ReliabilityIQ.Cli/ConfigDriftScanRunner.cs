using ReliabilityIQ.Analyzers.ConfigDrift;
...
public static class ConfigDriftScanRunner
{
    public static async Task<int> ExecuteAsync(
        ConfigDriftScanOptions options,
        TextWriter output,
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var candidateFiles = files
                .Where(f => f.Category is FileCategory.Config or FileCategory.DeploymentArtifact)
                .Where(IsConfigFile)
                .ToList();

            var input = new List<ConfigDriftFileInput>(candidateFiles.Count);
            foreach (var file in candidateFiles)
            {
                cancellationToken.ThrowIfCancellationRequested();
                var content = await File.ReadAllTextAsync(file.FullPath, cancellationToken).ConfigureAwait(false);
                input.Add(new ConfigDriftFileInput(file.RelativePath, content));
            }

            var analyzer = new ConfigDriftAnalyzer();
            var findings = analyzer.AnalyzeRepository(input, config.ScanSettings, cancellationToken)
            ...
            await output.WriteLineAsync($"Config drift findings: {findings.Count}").ConfigureAwait(false);
        ...
        {
            await output.WriteLineAsync($"Config drift scan failed: {ex.Message}").ConfigureAwait(false);
            return 2;
        }
    }

    private static bool IsConfigFile(DiscoveredFile file)
    {
